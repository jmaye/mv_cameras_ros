...
def getColorMode(serial):
  rospy.wait_for_service("/mv_cameras_manager/" + serial + "/get_color_mode")
  try:
    request = rospy.ServiceProxy(
      "/mv_cameras_manager/" + serial + "/get_color_mode", GetColorMode)
    response = request()
    print "Color mode for %s is: %f" %(serial, response.colorMode)
  except rospy.ServiceException, exception:
    print "GetColorMode request failed for %s: %s" %(serial, exception)
